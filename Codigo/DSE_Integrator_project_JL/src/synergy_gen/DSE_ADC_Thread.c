...
const adc_cfg_t DSE_portreadADC_cfg =
{ .unit = 0,
  ...
  .p_callback = NULL,
  .p_context = &DSE_portreadADC,
  .scan_end_ipl = (BSP_IRQ_DISABLED),
  .scan_end_b_ipl = (BSP_IRQ_DISABLED),
  .calib_adc_skip = false, };
const adc_channel_cfg_t DSE_portreadADC_channel_cfg =
{ .scan_mask = (uint32_t) (
...
const adc_instance_t DSE_portreadADC =
{ .p_ctrl = &DSE_portreadADC_ctrl,
  .p_cfg = &DSE_portreadADC_cfg,
  .p_channel_cfg = &DSE_portreadADC_channel_cfg,
  .p_api = &g_adc_on_adc };
...
static const timer_on_gpt_cfg_t DSE_timer_readADC_extend =
{ .gtioca =
{ .output_enabled = false, .stop_level = GPT_PIN_LEVEL_LOW },
  .gtiocb =
  { .output_enabled = false, .stop_level = GPT_PIN_LEVEL_LOW } };
static const timer_cfg_t DSE_timer_readADC_cfg =
{ .mode = TIMER_MODE_PERIODIC, .period = 300, .unit = TIMER_UNIT_PERIOD_MSEC, .duty_cycle = 50, .duty_cycle_unit =
          TIMER_PWM_UNIT_RAW_COUNTS,
  .channel = 0, .autostart = true, .p_callback = DSE_callback_timerADC, .p_context = &DSE_timer_readADC, .p_extend =
          &DSE_timer_readADC_extend,
  .irq_ipl = (6), };
/* Instance structure to use this module. */
const timer_instance_t DSE_timer_readADC =
{ .p_ctrl = &DSE_timer_readADC_ctrl, .p_cfg = &DSE_timer_readADC_cfg, .p_api = &g_timer_on_gpt };
...
void DSE_ADC_Thread_create(void)
{
    ...
    err_DSE_QueueADC = tx_queue_create (&DSE_QueueADC, (CHAR *) "DSE_QueueADC", 1, &queue_memory_DSE_QueueADC,
                                        sizeof(queue_memory_DSE_QueueADC));
    ...
    UINT err;
    err = tx_thread_create (&DSE_ADC_Thread, (CHAR *) "DSE_ADC_Thread", DSE_ADC_Thread_func, (ULONG) NULL,
                            &DSE_ADC_Thread_stack, 1024, 1, 1, 1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
