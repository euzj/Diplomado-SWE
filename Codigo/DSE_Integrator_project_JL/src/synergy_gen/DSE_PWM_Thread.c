...
static const external_irq_cfg_t dse_sensorHall_Interrup_cfg =
{ .channel = 0,
  .trigger = EXTERNAL_IRQ_TRIG_BOTH_EDGE,
  .filter_enable = true,
  .pclk_div = EXTERNAL_IRQ_PCLK_DIV_BY_64,
  .autostart = true,
  .p_callback = dse_sensorHall_CallBack,
  .p_context = &dse_sensorHall_Interrup,
  .p_extend = NULL,
  .irq_ipl = (4), };
/* Instance structure to use this module. */
const external_irq_instance_t dse_sensorHall_Interrup =
{ .p_ctrl = &dse_sensorHall_Interrup_ctrl, .p_cfg = &dse_sensorHall_Interrup_cfg, .p_api = &g_external_irq_on_icu };
...
static const timer_on_gpt_cfg_t dsw_timerControl_extend =
{ .gtioca =
{ .output_enabled = false, .stop_level = GPT_PIN_LEVEL_LOW },
  .gtiocb =
  { .output_enabled = false, .stop_level = GPT_PIN_LEVEL_LOW } };
static const timer_cfg_t dsw_timerControl_cfg =
{ .mode = TIMER_MODE_PERIODIC, .period = 25, .unit = TIMER_UNIT_PERIOD_MSEC, .duty_cycle = 50, .duty_cycle_unit =
          TIMER_PWM_UNIT_RAW_COUNTS,
  .channel = 2, .autostart = true, .p_callback = dsw_timerControl_CallBack, .p_context = &dsw_timerControl, .p_extend =
          &dsw_timerControl_extend,
  .irq_ipl = (5), };
/* Instance structure to use this module. */
const timer_instance_t dsw_timerControl =
{ .p_ctrl = &dsw_timerControl_ctrl, .p_cfg = &dsw_timerControl_cfg, .p_api = &g_timer_on_gpt };
...
static const timer_on_gpt_cfg_t dse_timerPWM_extend =
{ .gtioca =
{ .output_enabled = false, .stop_level = GPT_PIN_LEVEL_LOW },
  .gtiocb =
  { .output_enabled = true, .stop_level = GPT_PIN_LEVEL_LOW } };
static const timer_cfg_t dse_timerPWM_cfg =
{ .mode = TIMER_MODE_PWM,
  .period = 1,
  ...
  .autostart = true,
  .p_callback = NULL,
  .p_context = &dse_timerPWM,
  .p_extend = &dse_timerPWM_extend,
  .irq_ipl = (BSP_IRQ_DISABLED), };
/* Instance structure to use this module. */
const timer_instance_t dse_timerPWM =
{ .p_ctrl = &dse_timerPWM_ctrl, .p_cfg = &dse_timerPWM_cfg, .p_api = &g_timer_on_gpt };
...
void DSE_PWM_Thread_create(void)
{
    ...
    err_datadisplay = tx_queue_create (&datadisplay, (CHAR *) "datadisplay", 1, &queue_memory_datadisplay,
    ...
    UINT err;
    err = tx_thread_create (&DSE_PWM_Thread, (CHAR *) "DSE_PWM_Thread", DSE_PWM_Thread_func, (ULONG) NULL,
                            &DSE_PWM_Thread_stack, 1024, 1, 1, 1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
